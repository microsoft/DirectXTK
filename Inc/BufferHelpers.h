...
namespace DirectX
{
    ...
    template<typename T>
    HRESULT
    CreateStaticBuffer(_In_ ID3D11Device* device, T const& data, unsigned int bindFlags, _COM_Outptr_ ID3D11Buffer** pBuffer) noexcept
    {
    ...
    public:
        ...
        void Create(_In_ ID3D11Device* device) { CreateBuffer(device, sizeof(T), mConstantBuffer.ReleaseAndGetAddressOf()); }
