...
namespace DirectX
{
    ...
    {
    public:
        DIRECTX_TOOLKIT_API explicit AudioEngine(AUDIO_ENGINE_FLAGS flags    = AudioEngine_Default,
        ...
        DIRECTX_TOOLKIT_API bool __cdecl Reset(_In_opt_ const WAVEFORMATEX* wfx = nullptr, _In_opt_z_ const wchar_t* deviceId = nullptr);
        ...
        DIRECTX_TOOLKIT_API bool __cdecl Reset(_In_opt_ const WAVEFORMATEX* wfx = nullptr, _In_opt_z_ const __wchar_t* deviceId = nullptr);
#endif
    ...
    {
    public:
        DIRECTX_TOOLKIT_API WaveBank(_In_ AudioEngine* engine, _In_z_ const wchar_t* wbFileName);
        ...
        DIRECTX_TOOLKIT_API void __cdecl Play(unsigned int index, float volume, float pitch, float pan);

        DIRECTX_TOOLKIT_API void __cdecl Play(_In_z_ const char* name);
        DIRECTX_TOOLKIT_API void __cdecl Play(_In_z_ const char* name, float volume, float pitch, float pan);

        DIRECTX_TOOLKIT_API std::unique_ptr<SoundEffectInstance> __cdecl CreateInstance(unsigned int index,
            SOUND_EFFECT_INSTANCE_FLAGS                                                              flags = SoundEffectInstance_Default);
        DIRECTX_TOOLKIT_API std::unique_ptr<SoundEffectInstance> __cdecl CreateInstance(_In_z_ const char* name,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);

        DIRECTX_TOOLKIT_API std::unique_ptr<SoundStreamInstance> __cdecl CreateStreamInstance(unsigned int index,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);
        DIRECTX_TOOLKIT_API std::unique_ptr<SoundStreamInstance> __cdecl CreateStreamInstance(_In_z_ const char* name,
            SOUND_EFFECT_INSTANCE_FLAGS flags = SoundEffectInstance_Default);
        ...
        DIRECTX_TOOLKIT_API bool __cdecl GetPrivateData(unsigned int index, _Out_writes_bytes_(datasize) void* data, size_t datasize);

#if defined(_MSC_VER) && !defined(_NATIVE_WCHAR_T_DEFINED)
        DIRECTX_TOOLKIT_API WaveBank(_In_ AudioEngine* engine, _In_z_ const __wchar_t* wbFileName);
#endif
    ...
    {
    public:
        DIRECTX_TOOLKIT_API SoundEffect(_In_ AudioEngine* engine, _In_z_ const wchar_t* waveFileName);

        DIRECTX_TOOLKIT_API SoundEffect(_In_ AudioEngine* engine,
        ...
        DIRECTX_TOOLKIT_API SoundEffect(_In_ AudioEngine* engine,
            ...
            _In_reads_bytes_(audioBytes) const uint8_t* startAudio,
            size_t                                      audioBytes,
            _In_reads_(seekCount) const uint32_t*       seekTable,
            size_t                                      seekCount);
...
#ifdef USING_XAUDIO2_9
        DIRECTX_TOOLKIT_API bool __cdecl FillSubmitBuffer(_Out_ XAUDIO2_BUFFER& buffer, _Out_ XAUDIO2_BUFFER_WMA& wmaBuffer) const;
#else
...
#if defined(_MSC_VER) && !defined(_NATIVE_WCHAR_T_DEFINED)
        DIRECTX_TOOLKIT_API SoundEffect(_In_ AudioEngine* engine, _In_z_ const __wchar_t* waveFileName);
#endif
    ...
    struct DIRECTX_TOOLKIT_API AudioListener : public X3DAUDIO_LISTENER
    {
        ...
        void __cdecl SetOmnidirectional() noexcept { pCone = nullptr; }
    ...
    {
    public:
        DIRECTX_TOOLKIT_API DynamicSoundEffectInstance(_In_ AudioEngine* engine,
