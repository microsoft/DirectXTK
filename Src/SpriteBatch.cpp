...
{
    ThrowIfFailed(
        device->CreateVertexShader(SpriteEffect_SpriteVertexShader, sizeof(SpriteEffect_SpriteVertexShader), nullptr, &vertexShader));
    ...
    ThrowIfFailed(device->CreateInputLayout(VertexPositionColorTexture::InputElements,
...
SpriteBatch::Impl::RenderBatch(ID3D11ShaderResourceView* texture, SpriteInfo const* const* sprites, size_t count)
{
    auto deviceContext = mContextResources->deviceContext.Get();
    ...
    while (count > 0)
    {
...
#if defined(_XBOX_ONE) && defined(_TITLE)
        void* grfxMemory
            = GraphicsMemory::Get().Allocate(deviceContext, sizeof(VertexPositionColorTexture) * batchSize * VerticesPerSprite, 64);
        ...
        auto vertices
            = static_cast<VertexPositionColorTexture*>(mappedBuffer.pData) + mContextResources->vertexBufferPosition * VerticesPerSprite;
#endif
...
XMVECTOR SpriteBatch::Impl::GetTextureSize(_In_ ID3D11ShaderResourceView* texture)
{
    ...
    const XMVECTOR size = XMVectorMergeXY(XMLoadInt(&desc.Width), XMLoadInt(&desc.Height));
...
_Use_decl_annotations_ void SpriteBatch::GetViewportTransform(ID3D11DeviceContext* deviceContext, XMMATRIX& transformMatrix) const
{
